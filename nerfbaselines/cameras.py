...
def _iterative_undistortion(distortion: _DistortionFunction, uv: TTensor, params: TTensor, num_iterations: int = 100, **kwargs) -> TTensor:
    ...
    for i in range(num_iterations):
        ...
        J = xnp.stack(
            ...
            -1,
        ).reshape((*dx.shape, 2)).cuda() #torch
        step_x = xnp.linalg.solve(J+xnp.eye(2).cuda()*1e-13, (x + dx - uv)[..., None]).squeeze(-1)
        x -= step_x
    ...
    if camera_models.item() == camera_model_to_int("opencv_fisheye"):
        ...
        if distortion_parameters.shape[-1] != 0:
            uv[..., 0], uv[..., 1] = _radial_and_tangential_undistort(
                ...
                k3=distortion_parameters[...,2].item(),
                k4=distortion_parameters[...,3].item(),
                p1=distortion_parameters[...,4].item(),
                p2=distortion_parameters[...,5].item(),
                max_iterations=10,
                xnp=xnp,
            )
