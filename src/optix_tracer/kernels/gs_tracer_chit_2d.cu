...
extern "C" __global__ void __raygen__rg() {
    ...
    unsigned int prim_id = __UINT32_MAX__;
    unsigned int tmax;
    ...
    while(T > Tmin){
        optixTrace(
            ...
            prim_id,tmax,p[3]);
        if(prim_id == __UINT32_MAX__) break;
        const unsigned int hitParticle = prim_id/triagPerParticle;
        float resp,thit; 
        computeResponse(hitParticle,resp,thit,ray_origin,ray_direction);
        if(resp > respMin){
            const float3 rad = computeRadiance(hitParticle,ray_origin); 
            radiance += rad*resp*T;
            T *= (1.-resp);
        }
        min_dist = __uint_as_float(tmax)+eps;
        prim_id = __UINT32_MAX__;
    }
    //printf("%d\n",k);
