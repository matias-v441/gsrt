...
__device__ float3 computeCov2D(const float3& mean, float focal_x, float focal_y, float tan_fovx, float tan_fovy, const float* cov3D, const float* viewmatrix, const bool is_fisheye)
{
	...
	if (is_fisheye) // fisheye
	{
		...
		float b = glm::atan(len_xy, t.z + 0.0000001f) / len_xy / x2y2;
	...
	bool prefiltered)
{
	...
	if (is_fisheye) // panorama
	{
		...
		//if (abs(theta) > 3.14 * 0.403)return; // restricts to 72.5 deg. >0.5 cut by frustum
		//if (abs(theta) > 3.14 * 0.5)return; // restricts to 72.5 deg. >0.5 cut by frustum
		// if(abs(theta-3.14/2)<0.01)return; // crease around 90 deg due to z sorting
		p_proj.x = 2 * p_view.x * focal_x * theta / (xy_len * W);
		p_proj.y = 2 * p_view.y * focal_y * theta / (xy_len * H);
		//p_proj.x = p_view.x * focal_x * theta / xy_len; -> ndc
		//p_proj.z = p_view.y * focal_y * theta / xy_len;
	}
	else
	{
