...
class GaussianSplatting(Method):
    def __init__(self, *, unfisheye=False,
        ...
        if train_dataset:
            self.train_cameras_th = train_dataset['cameras'].apply(fit_distortion) 
        self.unfisheye = unfisheye
    ...
    def render(self, cameras: Cameras, *, options=None) -> RenderOutput:
        camera = cameras.item()
        #assert np.all(camera.camera_models == camera_model_to_int("pinhole")), "Only pinhole cameras supported"
        print(camera.intrinsics)
        is_fisheye = self.train_cameras_th.camera_models[0] == camera_model_to_int("opencv_fisheye")
        with torch.no_grad():
            ...
            image = torch.clamp(render(viewpoint, self.gaussians, self.pipe, self.background, is_fisheye=is_fisheye)["render"], 0.0, 1.0)
            if is_fisheye:
                def fit_distortion(x,name):
                    ...
                    return torch.from_numpy(x).contiguous().cuda()
                camera_th = cameras.apply(fit_distortion).item()
                ...
                print(camera_th.distortion_parameters)
                image_undist = self.undistort_image(image, camera_th)
                color = image_undist.detach()
            else:
    ...
    def undistort_image(self, image, viewpoint_cam_nbs, display=False, cam_id=0, iteration=0):
        ...
        xy[:,0] = (xy[:,0] - cx)/fx
        xy[:,1] = (xy[:,1] - cy)/fy
        xy[:,0], xy[:,1] = _radial_and_tangential_undistort(
            xy[:,0],
            xy[:,1],
            k1=viewpoint_cam_nbs.distortion_parameters[...,0].item(),
            k2=viewpoint_cam_nbs.distortion_parameters[...,1].item(),
            k3=viewpoint_cam_nbs.distortion_parameters[...,2].item(),
            k4=viewpoint_cam_nbs.distortion_parameters[...,3].item(),
            p1=viewpoint_cam_nbs.distortion_parameters[...,4].item(),
            p2=viewpoint_cam_nbs.distortion_parameters[...,5].item(),
            max_iterations=10,
            xnp=torch,
        )
        if self.unfisheye:
            th = torch.min(torch.sqrt(xy[:, 0] ** 2 + xy[:, 1] ** 2),torch.tensor(torch.pi))
            tth = torch.tan(th)
            xy[:,0] = xy[:,0]/th*tth
            xy[:,1] = xy[:,1]/th*tth
        xy[:,0] = xy[:,0]*fx+cx
        xy[:,1] = xy[:,1]*fy+cy
        res_y,res_x = image.shape[1:]
        image_undist = interpolate_bilinear(image.permute(1,2,0), xy).reshape(res_y,res_x,3)
        if display:
            import matplotlib.pyplot as plt
            plt.figure()
            plt.axis('off')
            plt.imshow(image.detach().permute(1,2,0).cpu().numpy())
            plt.scatter(xy.cpu().numpy()[:,0],xy.cpu().numpy()[:,1],s=0.005)
            plt.savefig(f"distort_id{cam_id}_it{iteration}.png",bbox_inches="tight",pad_inches=0)
            plt.figure()
            plt.axis('off')
            plt.imshow(image_undist.detach().cpu().numpy())
            plt.savefig(f"distorted_id{cam_id}_it{iteration}.png",bbox_inches="tight",pad_inches=0)
        return image_undist
    ...
    def train_iteration(self, step):
        ...
        if not self._viewpoint_stack:
            ...
            self._viewpoint_stack_nbs = torch.arange(len(self.train_cameras_th)).tolist()
            if any(not getattr(cam, "_patched", False) for cam in self._viewpoint_stack):
                raise RuntimeError("could not patch loadCam!")
        cam_id = randint(0, len(self._viewpoint_stack) - 1)
        viewpoint_cam = self._viewpoint_stack.pop(cam_id)
        nbs_cam_id = self._viewpoint_stack_nbs.pop(cam_id)
        viewpoint_cam_nbs = self.train_cameras_th.__getitem__(nbs_cam_id)

        # Render
        bg = torch.rand((3), device="cuda") if self.opt.random_background else self.background

        is_fisheye = viewpoint_cam_nbs.camera_models.item() == camera_model_to_int("opencv_fisheye")
        use_fisheye = is_fisheye and not self.unfisheye
        render_pkg = render(viewpoint_cam, self.gaussians, self.pipe, bg, is_fisheye=use_fisheye)
        image, viewspace_point_tensor, visibility_filter, radii = render_pkg["render"], render_pkg["viewspace_points"], render_pkg["visibility_filter"], render_pkg["radii"]

        display = nbs_cam_id in [587] 
        if is_fisheye:
            image_undist = self.undistort_image(image, viewpoint_cam_nbs, display, nbs_cam_id, iteration)
        ...
        if display:
            ...
            plt.axis('off')
            plt.imshow(image.detach().permute(1,2,0).cpu().numpy())
            plt.savefig(f"original_id{nbs_cam_id}_it{iteration}.png",bbox_inches="tight",pad_inches=0)
            plt.figure()
            plt.axis('off')
            plt.imshow(gt_image.detach().permute(1,2,0).cpu().numpy())
            plt.savefig(f"gt_{nbs_cam_id}.png",bbox_inches="tight",pad_inches=0)
            #plt.show()
        if is_fisheye:
            image = image_undist.permute(2,0,1)
        ...
        pos_grad_norm = torch.norm(xyz.grad,dim=1) if xyz.grad is not None else torch.zeros(xyz.shape[0])
        ...
        with torch.no_grad():
            ...
            metrics = {
                ...
                "densif_stats": self.gaussians.stats,
            ...
            if iteration < self.opt.densify_until_iter:
                ...
                if iteration > self.opt.densify_from_iter and iteration % self.opt.densification_interval == 0:
                    ...
                    self.gaussians.densify_and_prune(self.opt.densify_grad_threshold, 0.005, self.scene.cameras_extent, size_threshold, iteration)
                    #print(f"XYZ GRAD {self.gaussians._xyz.grad}")

                if iteration % self.opt.opacity_reset_interval == 0 or (self.dataset.white_background and iteration == self.opt.densify_from_iter):
                    self.gaussians.reset_opacity()

            #print(self.opt.densify_from_iter)
            #print(f'{iteration} after densif {self.gaussians.get_xyz.shape}')
