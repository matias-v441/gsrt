...
def create_fisheye_camera(principal_point, focal_length, radial_coeffs, pixel_coords, dev, w, h):
    ...
    #pixel_coords = torch.tensor(np.stack([u, v], axis=1), dtype=torch.int32, device=dev)
    image_points = pixels_to_image_points(pixel_coords)
    rays_d_cam = image_points_to_camera_rays(params, image_points, device=dev)
    return rays_d_cam