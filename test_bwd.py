...
part_scale = torch.tensor([[1.,1.,2.]]).repeat(part_num,1)
...
check = torch.autograd.gradcheck(trace_function_check,
                                 (part_opac,part_xyz,part_scale,part_rot,part_sh),
                                 nondet_tol=1e-7,
                                 atol=1e-3,
                                 eps=1e-1
                                 )
...
for it in range(10000):
    ...
    L = trace_function_check(part_opac_,part_xyz,part_scale_,part_rot,part_sh)
    print(L)
    L.backward()
    ...
    if L < best_loss:
        best_loss = L.detach()
        best_model = (x.clone() for x in (part_opac,part_xyz,part_rot,part_scale,part_sh))

print("MSE ", best_loss)
part_opac,part_xyz,part_rot,part_scale,part_sh = best_model
